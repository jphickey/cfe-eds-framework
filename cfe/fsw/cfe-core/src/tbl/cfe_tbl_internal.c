                           ...
                           const char *Filename)
{
    ...
    if ((TblFileHeader.Offset + TblFileHeader.NumBytes) > RegRecPtr->BinaryFileSize)
    {
        CFE_EVS_SendEventWithAppID(CFE_TBL_LOAD_EXCEEDS_SIZE_ERR_EID,
            ...
            (long unsigned int)RegRecPtr->BinaryFileSize);
...
int32 CFE_TBL_DecodeFromMemory(const void *SourceBuffer, CFE_TBL_LoadBuff_t *WorkingBufferPtr,
        CFE_TBL_RegistryRec_t *RegRecPtr)
{
    EdsLib_Id_t          FinalId;
    int32       Status;
    ...
    Status = EdsLib_DataTypeDB_UnpackPartialObject(CFE_SB_GetEds(), &FinalId, WorkingBufferPtr->BufferPtr,
            SourceBuffer, RegRecPtr->EdsInfo.Size.Bytes, 8 * RegRecPtr->BinaryFileSize, 0);

    if (Status == EDSLIB_SUCCESS)
    {
        Status = EdsLib_DataTypeDB_VerifyUnpackedObject(CFE_SB_GetEds(), FinalId,
                WorkingBufferPtr->BufferPtr, SourceBuffer,
